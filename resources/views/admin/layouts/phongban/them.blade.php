...
@section('title', 'Thêm phòng ban - Giám đốc')
...
<div class="space-y-5">
    <div class="pb-hero">
        <div>
            <div class="pb-kicker">New department</div>
            <h1 class="pb-title">Tạo phòng ban mới</h1>
            <p class="pb-desc">Phòng ban mới sẽ dùng cho quản lý nhân sự, phạm vi giao việc và báo cáo tiến độ.</p>
        </div>
        <a href="{{ route('phongban.danhsach') }}" class="pb-btn pb-btn-secondary"><i data-lucide="arrow-left"></i> Quay lại</a>
    </div>

    <div class="pb-panel">
        <div class="pb-panel-head"><i data-lucide="building"></i> Thông tin phòng ban</div>
        <form method="POST" action="{{ route('phongban.luu') }}" class="pb-form">
            @csrf
            @if ($errors->any())
                <div class="pb-error">
            ...
            <div class="pb-field">
                ...
                <input class="pb-input" name="TENPHONG" value="{{ old('TENPHONG') }}" required placeholder="VD: Nhân sự">
            </div>
            <div class="pb-actions">
                <a href="{{ route('phongban.danhsach') }}" class="pb-btn pb-btn-secondary">Hủy</a>
                <button class="pb-btn pb-btn-primary" type="submit"><i data-lucide="save"></i> Tạo phòng ban</button>
            </div>
        </form>
    </div>
</div>
